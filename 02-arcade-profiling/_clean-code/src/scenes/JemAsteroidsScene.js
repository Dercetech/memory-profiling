...
export class JemAsteroidsScene extends Phaser.Scene {
  ...
  createObjectGroups() {
    ...
    this.populatePool(this.bullets, Bullet, BULLETS.poolSize);
    this.populatePool(this.specialParticles, SpecialEnergyParticle, SPECIAL_ATTACK.poolSize);
    this.populatePool(this.explosionDebris, ExplosionParticle, EXPLOSIONS.poolSize);
  }

  populatePool(group, ObjectType, size) {
    for (let index = 0; index < size; index += 1) {
      const object = new ObjectType(this);
  ...
  hitSpecialParticleEnemy(particle, enemy) {
    particle.deactivate();
    this.destroyEnemy(enemy);
  }
  ...
  fireBullet(time) {
    ...
    if (bullet && this.capacitor.spend(BULLETS.capacitorCost)) {
  ...
  emitExplosion(x, y) {
    ...
    for (let index = 0; index < EXPLOSIONS.particlesPerMine; index += 1) {
      const particle = this.explosionDebris.getFirstDead(false);
